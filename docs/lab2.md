...
然后接下来就会发生syscall,去内核态找这个系统调用函数，这里syscall会接受一点参数，主要是匹配一下你要干什么操作，你给一个id，然后做对应的活，之后就是一大堆你传入的参数,把那些传进来(这里有个小偷懒的技巧，如果你不知道参数是什么类型，去找到那个函数的定义，然后 as 某某类型),这里也是照猫画虎
